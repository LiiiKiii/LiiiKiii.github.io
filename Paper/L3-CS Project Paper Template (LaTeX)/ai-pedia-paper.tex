...
The figure therefore highlights two distinct roles of the recommendation stage. The first is \emph{compression}: it reduces 75 candidates to 15 visible outputs. The second is \emph{pedagogical balancing}: it prevents the system from simply returning the most common type of result. In the present implementation, this balancing happens by selecting top resources per modality rather than globally ranking all resources together. This strategy is intentionally simple, but it aligns well with the educational motivation of the project, which is not just to find relevant resources but to provide complementary forms of reinforcement.
...
\texttt{test/evaluation\_pipeline/evaluator.py} & Rewrote the evaluator so it can actually run end-to-end and export JSON plus chart outputs. & Makes the reported results reproducible and directly links the codebase to the paper’s tables and figures.\\
\texttt{test/performance/run\_performance\_tests.py} & Replaced placeholder timing logic with measurements of keyword extraction, ranking, and combined local pipeline stages. & Provides genuine implementation-level performance evidence instead of superficial script execution.\\
\hline\hline
\end{tabular}
\end{table*}
...
\hline
Keyword extraction & 0.15 & 141.88 & TF-IDF + MMR only\\
Ranking pipeline & 0.17 & 142.73 & CBF over cached resources\\
Local combined pipeline & 0.18 & 143.00 & Excludes web search latency\\
\hline\hline
\end{tabular}
\end{table}
...
\item \textbf{Resource AI relevance}: percentage of recommended items whose title, description, or URL contains AI/ML evidence;
...
Table~\ref{tab:pipeline_comparison} reports the main resource-quality comparison. The raw search stage returns 75 resources across three modalities. Reducing this set to a balanced top-five-per-type recommendation list yields an 80\% reduction in candidate volume, which is substantial from a user-experience perspective because it transforms a large and noisy set into a small, manageable reinforcement package.
...
\hline
Total resources & 75 & 15 & 15\\
AI relevance (\%) & 81.33 & 93.33 & 86.67\\
Authority score (\%) & 41.33 & 40.00 & 60.00\\
Valid URLs (\%) & 100.00 & 100.00 & 100.00\\
Noise reduction (\%) & 0.00 & 80.00 & 80.00\\
Cross-platform diversity & 3 & 3 & 3\\
\hline\hline
\end{tabular}
\end{table}

Two observations are important here. First, even the raw search stage already has relatively high AI relevance (81.33\%), which indicates that the system’s query construction and search-source design are directionally correct. Second, the ranking stage improves \emph{authority} much more clearly than it improves every other metric. Specifically, authority rises from 40.00\% in the unranked top-$K$ set to 60.00\% in the CBF-ranked set, while valid URLs remain at 100\%. In other words, ranking does not simply preserve relevance; it shifts the recommendation list toward more trustworthy sources.

The slight drop in AI relevance from 93.33\% to 86.67\% between the unranked and ranked top-$K$ sets should not be hidden. On this small pilot corpus, the first-returned items for some queries are already strongly AI-themed, so a pure AI-keyword percentage favours the unranked set. The ranked set nevertheless appears stronger on source quality, which is desirable for an educational recommender. This is a useful design insight rather than a failure: on a small corpus, authority and topical specificity can trade off against one another, and the current system already exposes that tradeoff in a measurable way.

\begin{figure}[!t]
\centering
\includegraphics[width=0.45\textwidth]{figures/evaluation_pipeline_comparison.png}
\caption{Comparison of raw search, unranked top-$K$, and CBF-ranked top-$K$ outputs. Ranking preserves a compact final set and improves source authority, while the main compression effect comes from reducing the candidate pool from 75 resources to 15.}
