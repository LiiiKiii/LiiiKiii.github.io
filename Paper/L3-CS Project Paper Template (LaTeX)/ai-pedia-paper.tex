...
\hline
\texttt{recommender.py} & Removed summary generation during intermediate recommendation, added lightweight domain bonus, stored final \texttt{ranking\_score}. & Avoids duplicate API work, makes ranking more transparent, and improves the authority profile of the final list.\\
...
\texttt{keyword\_extractor.py} & Added redundancy pruning after MMR selection. & Prevents phrases such as short/long containment variants from crowding the keyword set and improves search-query usefulness.\\
\texttt{metrics.py} & Reworked metrics to cover coverage, diversity, AI relevance, authority, novelty, URL validity, and modality counts. & Turns evaluation from an informal inspection task into a repeatable metric-based process suitable for inclusion in the dissertation.\\
\texttt{resource\_searcher.py} & Added a shared HTTP session with retry-aware GET handling for repeated source queries. & Reduces repeated connection overhead and makes retrieval behaviour more stable across evaluation reruns.\\
\texttt{evaluator.py} & Rewrote the evaluator so it can actually run end-to-end and export JSON, CSV, LaTeX tables, and chart outputs. & Makes the reported results reproducible and directly links the codebase to the paper’s tables and figures.\\
\texttt{run\_performance\_tests.py} & Replaced placeholder timing logic with measurements of keyword extraction, ranking, and combined local pipeline stages, and exported paper-ready artefacts. & Provides genuine implementation-level performance evidence instead of superficial script execution.\\
\hline\hline
\end{tabular}
\end{table*}
