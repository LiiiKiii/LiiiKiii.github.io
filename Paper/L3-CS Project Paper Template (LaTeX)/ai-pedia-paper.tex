...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.45\textwidth]{figures/resource_type_distribution.png}
\caption{Distribution of text, video, and code resources before and after recommendation compression.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.45\textwidth]{figures/local_pipeline_performance.png}
...
\section{Evaluation}
    ...
    This section evaluates the AI-Pedia system using a \textbf{holdout validation} approach inspired by machine learning's train/validation split methodology. The evaluation aims to determine whether AI-Pedia's structured pipeline produces qualitatively different and better-calibrated recommendations compared to a free-form LLM baseline.

\subsection{Holdout Validation Framework}

    The holdout validation framework adapts the train/validation split concept from machine learning to educational resource recommendation. Unlike a traditional ML evaluation where a model is trained on training data and evaluated on held-out validation data, our framework uses:

\begin{itemize}
    \item \textbf{Input Set ($\sim$20 documents)}: Documents used by both systems to generate recommendations. These simulate a learner's private study corpus.
    \item \textbf{Validation Set ($\sim$50 documents)}: Independent documents used to evaluate recommendation quality. These represent the broader knowledge space that recommendations should address.
    \item \textbf{Comparison}: Recommendations from both systems are evaluated against the validation set using independent metrics.
\end{itemize}

    This approach avoids self-referential evaluation because the validation set is completely independent of both systems.

    In our setup, each topic domain contains 10 input documents and 10 validation documents. While smaller than a production-scale evaluation, this setup is sufficient to demonstrate the structured retrieval advantage: the input documents simulate a learner's private study corpus, while the validation documents represent the broader knowledge space that recommendations should address.

\subsection{Comparison Methods}

    We compare two approaches, both of which use LLM capabilities, but with different levels of structure:

\subsubsection{Baseline: Free-form LLM with Web Search}
    This baseline simulates a learner who has web search capability but lacks structured retrieval tools. The LLM (GPT-4o with browsing tool-use) can freely search the web, but has no:
\begin{itemize}
    \item Structured keyword extraction (no TF-IDF, no MMR)
    \item Predefined source whitelist (any domain is acceptable)
    \item Modality balancing (any resource type)
    \item Content-based ranking (returns what the browser finds)
\end{itemize}

    This represents an unconstrained approach where the LLM decides what to search, where to search, and how to present results.

\subsubsection{AI-Pedia: Structured Pipeline}
    The full AI-Pedia system implements a structured pipeline:
\begin{enumerate}
    \item \textbf{TF-IDF + MMR keyword extraction}: Extracts topic-bearing phrases from the input corpus
    \item \textbf{Multi-source retrieval}: Searches predefined high-quality sources (Wikipedia, arXiv, Scholar, YouTube, GitHub)
    \item \textbf{Content-Based Filtering (CBF) ranking}: Ranks resources by similarity to the input corpus
\end{enumerate}

    This represents a constrained, interpretable approach where structure guides retrieval.

\subsection{Evaluation Metrics}

    We use two complementary evaluation dimensions:

\subsubsection{Simpson's Diversity Index}
    Simpson's Diversity Index measures how evenly resources are distributed across categories:

$$D = 1 - \sum_{i=1}^{R} p_i^2$$

    where $p_i$ is the proportion of resources in category $i$. $D=1$ means perfect evenness (maximum diversity); $D=0$ means all resources are in one category (no diversity).

    We compute diversity across:
\begin{itemize}
    \item \textbf{Domain diversity}: Distribution across source domains (academic, tutorial, code repositories, video platforms)
    \item \textbf{Type diversity}: Distribution across resource types (text, video, code)
    \item \textbf{Authority diversity}: Distribution across authority categories (academic papers, tutorials, community resources)
\end{itemize}

    Simpson's D is computed from observed frequencies and requires no quality assumptions about any particular domain or type.

\subsubsection{Validation Set Relevance}
    We evaluate whether recommendations actually address topics covered in the validation set using an \textbf{independent embedding model} (Sentence-BERT, distinct from AI-Pedia's TF-IDF) to avoid self-referential bias:

\begin{itemize}
    \item \textbf{Mean similarity}: Average cosine similarity between recommendations and validation documents
    \item \textbf{Coverage score}: Proportion of validation documents with at least one relevant recommendation (similarity > 0.3)
\end{itemize}

    This metric measures whether recommendations cover the broader knowledge space implied by the validation set, not just the input corpus.

\subsection{Experimental Setup}

    We evaluate across multiple AI/ML topic domains, each with an input set of 10 documents and a validation set of 10 documents. The evaluator (\texttt{test/holdout\_eval/holdout\_evaluator.py}) produces JSON results, CSV summaries, LaTeX tables, and comparison plots automatically.
...
\subsection{Analysis}
    ...
    \textbf{1. AI-Pedia produces more diverse recommendations.} Across all topic domains, AI-Pedia's structured pipeline achieves higher Simpson's Diversity Index ($D_{\text{AI-Pedia}} = 0.818$) compared to the free-form LLM baseline ($D_{\text{baseline}} = 0.773$). More importantly, AI-Pedia significantly improves authority diversity ($D_{\text{authority}}$ from 0.560 to 0.773, a gain of +0.213). This reflects AI-Pedia's deliberate design choice to search across structured academic and tutorial sources, while free-form browsing naturally gravitates toward whichever domains rank highest in general web search (predominantly blog/tutorial platforms).

    \textbf{2. AI-Pedia produces better-calibrated resource distributions.} The baseline tends to return resources concentrated in blog platforms (medium.com, towardsdatascience.com accounting for 53\% of results), while AI-Pedia distributes resources more evenly across Wikipedia (13\%), arXiv (20\%), YouTube (27\%), GitHub (20\%), and tutorial sites (27\%). This structured distribution better serves educational purposes by ensuring learners access authoritative and diverse resource types.

    \textbf{3. Authority coverage is significantly improved.} AI-Pedia achieves 73\% authority coverage (recommendations from trusted academic/tutorial sources), compared to only 13\% for the baseline which returns mostly blog-platform content. This demonstrates that structured retrieval effectively guides learners toward higher-quality educational resources.

    \textbf{4. Modality balance is improved with structured retrieval.} AI-Pedia achieves a modality balance score of 0.800 compared to the baseline's 0.733. This indicates that the structured approach maintains better balance across text, video, and code resources, ensuring learners receive complementary forms of reinforcement rather than being overwhelmed by any single type.

These findings support the central hypothesis: structured retrieval, guided by principled keyword extraction and multi-source search, produces qualitatively different and better-calibrated recommendations than free-form LLM browsing for educational purposes.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.42\textwidth]{figures/holdout_diversity.png}
    \caption{Domain Diversity: Free-form LLM Baseline vs AI-Pedia Structured Pipeline.}
    \label{fig:holdout_diversity}
\end{figure}

These results are summarised in Table~\ref{tab:holdout_results}. Figure~\ref{fig:holdout_diversity} visualises the Simpson's Diversity comparison across the three evaluation corpora.
    ...
    \textbf{3. Authority coverage is significantly improved.} AI-Pedia achieves 100\% authority coverage (all recommendations from trusted academic/tutorial sources), compared to 0\% for the baseline which returns mostly blog-platform content. This demonstrates that structured retrieval effectively guides learners toward higher-quality educational resources.
...
Figure~\ref{fig:holdout_relevance} illustrates the validation set relevance comparison, showing that AI-Pedia's structured pipeline produces recommendations that are more relevant to the broader knowledge space implied by the validation documents.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.42\textwidth]{figures/holdout_relevance.png}
    \caption{Validation Set Relevance: Free-form LLM vs AI-Pedia.}
...
\begin{figure}[htbp]
    ...
    \caption{Authority Coverage: Percentage of recommended resources from trusted academic and tutorial sources.}
