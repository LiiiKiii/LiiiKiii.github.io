...
To validate the end-to-end workflow, the current prototype was exercised on three focused AI/ML corpora bundled with the project. The first corpus, \emph{foundations\_ml}, contains ten documents centred on introductory machine learning concepts such as supervised learning, optimisation, feature engineering, and model evaluation. The second corpus, \emph{nlp\_transformers}, contains ten documents centred on modern NLP topics such as tokenization, attention, transformers, language-model pretraining, and retrieval-augmented generation. The third corpus, \emph{vision\_representation}, contains ten documents centred on computer vision topics such as convolutional networks, image classification, object detection, transfer learning, and visual representation learning. Across the three corpora, the total token count is 4,528 words. Each corpus individually satisfies the system's minimum ten-document upload threshold while remaining coherent enough to represent a realistic learner scenario.
The documents were selected deliberately to preserve topical focus inside each corpus rather than mixing unrelated AI directions into one bundle. In other words, the evaluation no longer relies on a single topic-diverse corpus; it now uses three controlled ten-document corpora that better match how the recommender is supposed to infer learning intent from a coherent input set.

In the current focused-corpus setup, each corpus produces ten final topic phrases after TF-IDF weighting, MMR selection, and post-processing for redundancy reduction. The foundations corpus yields phrases such as \emph{machine learning}, \emph{model evaluation}, and \emph{feature engineering}; the NLP corpus yields phrases such as \emph{natural language processing}, \emph{attention mechanisms}, and \emph{large language models}; and the vision corpus yields phrases such as \emph{image classification}, \emph{transfer learning}, and \emph{medical imaging}. Across the three corpora, multi-source retrieval returned 542 unique resources after de-duplication, comprising 257 text items, 210 videos, and 75 code repositories or notebooks. After recommendation and balancing, the final exported output contained 45 resources in total, with each corpus contributing a 15-item multimodal recommendation package.

\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Focused Multi-Corpus Evaluation Setup and Aggregated Outputs}
...
Metric & Value\\
\hline
Corpus setting & 3 corpora (30 documents total)\\
Total corpus words & 4,528\\
Final extracted keywords & 10 per corpus\\
Raw text resources & 257\\
Raw video resources & 210\\
Raw code resources & 75\\
Final recommended text resources & 15\\
Final recommended video resources & 15\\
Final recommended code resources & 15\\
Total final recommended items & 45\\
\hline\hline
\end{tabular}
\end{table}
...
The figure therefore highlights two distinct roles of the recommendation stage. The first is \emph{compression}: it reduces 542 candidates to 45 visible outputs across the three corpora. The second is \emph{pedagogical balancing}: it prevents the system from simply returning the most common type of result. In the present implementation, this balancing happens by selecting top resources per modality within each corpus rather than globally ranking all resources together. This strategy is intentionally simple, but it aligns well with the educational motivation of the project, which is not just to find relevant resources but to provide complementary forms of reinforcement.
...
\hline
Focused corpora & Supply three coherent ten-document AI/ML corpora for repeatable end-to-end testing.\\
...
Table~\ref{tab:keyword_quality} summarises the aggregate keyword comparison across the three focused corpora. The naive frequency baseline preserves full document coverage, while the TF-IDF + MMR pipeline sacrifices some literal coverage (100\% to 86.67\%) in exchange for a stronger AI-domain signal, raising keyword AI relevance from 13.33\% to 30.74\%. Across all three corpora, the implemented method produces more topic-bearing multi-word phrases than the unigram baseline.

\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Keyword Extraction Aggregated Across Three Focused 10-Document Corpora}
...
The selected phrases from the AI-Pedia pipeline are more descriptive than the unigram baseline. Instead of returning broad words such as \emph{learning}, \emph{models}, or \emph{text}, the implemented method surfaces topic-bearing phrases such as \emph{model evaluation}, \emph{attention mechanisms}, \emph{large language models}, and \emph{transfer learning}. This is an important qualitative outcome, because the downstream search stage benefits from queries that already encode concept boundaries within each focused corpus. The lexical diversity score of the phrase-based method is slightly lower than that of the unigram baseline, and the coverage proxy also drops on the more specialised corpora because a smaller set of specific phrases cannot literally touch every document. That should not be over-interpreted as a failure. In this setting, lower literal coverage is the cost of replacing generic tokens with sharper, retrieval-oriented topic phrases. In practice, the phrase-based output is more usable for retrieval because it encodes topic structure rather than generic high-frequency tokens.

\begin{figure}[!t]
\centering
\includegraphics[width=0.45\textwidth]{figures/evaluation_precision_comparison.png}
\caption{Keyword extraction comparison between a simple frequency baseline and the implemented TF-IDF + MMR pipeline, aggregated across three focused corpora. The implemented method improves AI-domain focus and produces more meaningful topic phrases, although it sacrifices some literal coverage on the more specialised corpora.}
\label{fig:keyword_comparison}
\end{figure}

\subsection{Resource Quality After Ranking (RQ2 and RQ3)}

Table~\ref{tab:pipeline_comparison} reports the aggregate resource-quality comparison across all three focused corpora. The raw search stage returns 542 resources across three modalities. Reducing this set to a balanced top-five-per-type recommendation list for each corpus yields an overall 91.70\% reduction in candidate volume, which is substantial from a user-experience perspective because it transforms a large and noisy set into three small, manageable reinforcement packages.
...
Platform diversity & 3 & 3 & 3\\
\hline\hline
\end{tabular}
\end{table*}

Two observations are important here. First, even the raw search stage already has high AI relevance (78.41\%), which indicates that the system’s query construction and search-source design are directionally correct across all three focused learner scenarios. Second, the ranking stage improves both \emph{authority} and measured AI relevance in the final pooled top-$K$ set. Specifically, authority rises from 40.00\% in the unranked top-$K$ set to 66.67\% in the CBF-ranked set, while AI relevance increases from 86.67\% to 93.33\% and valid URLs remain at 100\%.
...
\caption{Comparison of raw search, unranked top-$K$, and CBF-ranked top-$K$ outputs, aggregated across three focused corpora. Ranking improves both AI relevance and source authority, and the main compression effect comes from reducing the candidate pool from 542 resources to 45.}
\label{fig:pipeline_comparison}
\end{figure}

\subsection{Interpretation and Limitations of the Current Evaluation}

Taken together, the current focused-corpus evaluation supports three preliminary conclusions. For \textbf{RQ1}, the implemented keyword extraction stage is more domain-focused than a naive frequency baseline, even though its higher phrase specificity slightly reduces a literal coverage proxy. For \textbf{RQ2}, multi-source retrieval successfully produces a broad candidate pool spanning text, video, and code resources across three learner scenarios, and the final recommendation stage reduces that pool to balanced output sets suitable for user consumption. For \textbf{RQ3}, CBF-based ranking again appears most valuable as a \emph{quality-shaping} mechanism, particularly by increasing the proportion of authoritative sources in the final result set.

At the same time, this evaluation remains a pilot rather than a final summative study. The setup now uses three focused corpora that each meet the system's ten-document input threshold, but the overall benchmark is still relatively small, the web sources are live and therefore volatile, and no human relevance judgements have yet been collected. In addition, metrics such as keyword diversity and AI-keyword matching are still proxy measures rather than substitutes for expert assessment. The current results should therefore be interpreted as evidence that the implementation is functioning coherently and that the main design choices are measurable, not as the final word on educational effectiveness.

The next stage of evaluation should extend this pilot in three directions. First, the benchmark should be enlarged beyond the current three focused corpora and expanded into additional topic clusters such as reinforcement learning, graph learning, or multimodal study packs. Second, human annotations should be added for relevance, usefulness, and learning value. Third, a controlled user study should compare AI-Pedia with manual search or other learning-support tools. Even so, the present pilot is already valuable because it converts the paper’s claims from purely architectural statements into reproducible, code-backed observations.
...
The focused multi-corpus evaluation revealed a useful shift between compression and quality improvement. Across the three ten-document corpora, the unranked top-$K$ resource sets already achieved strong AI relevance, but the final ranked sets improved the result further. In aggregate, the ranked output increased authority from 40.00\% to 66.67\% while also increasing measured AI relevance from 86.67\% to 93.33\%. In an educational setting, that combination is desirable: the recommendation stage is not merely filtering items down to smaller lists, but actively reshaping the output toward more trustworthy and more clearly AI-focused resources.
...
A second trade-off concerns phrase specificity versus lexical diversity and coverage in keyword extraction. The TF-IDF + MMR stage produced better topic phrases than the simple frequency baseline, but the lexical diversity score was slightly lower and the coverage proxy no longer remained perfect across all three corpora. Here again, the lower score does not necessarily indicate worse behaviour. Multi-word AI concepts often share component terms, and focused phrases can leave some documents without an explicit literal keyword hit even when retrieval quality improves. This result highlights the need to choose evaluation metrics that reflect downstream usefulness, not just surface-form variation.

\subsection{From Prototype to Deployable System}

From an engineering standpoint, the current codebase is now much closer to a deployable prototype than to a proof-of-concept script collection. The application has a working interface, modular backend components, exportable output folders, and dedicated evaluation and performance scripts. It also includes practical deployment assets, namely a Dockerfile, a Docker Compose configuration, an environment template, and a health-check endpoint, so that the full system can be started reproducibly rather than only as an ad hoc development script. More importantly, the code was refined so that responsibilities are better separated across modules: keyword extraction performs topic discovery, search modules perform retrieval, the recommender performs ranking, and the interface handles final display and lazy summary generation. This modularity makes the system easier to test, optimise, and explain.
...
Overall, the combination of code refinement and pilot evaluation strengthens the central claim of the project. AI-Pedia is not simply an idea for using AI in education; it is an implemented, inspectable, extensible, and deployable containerised prototype whose behaviour can already be analysed quantitatively. That makes it a solid basis for a final-year project and a credible foundation for future research or practical deployment.
