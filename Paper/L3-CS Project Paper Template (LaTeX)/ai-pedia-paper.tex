...
To validate the end-to-end workflow, the current prototype was exercised on a pilot AI/ML corpus bundled with the project. The corpus now contains ten text documents spanning machine learning fundamentals, neural networks, transformers, natural language processing, computer vision, optimisation, reinforcement learning, generalization and regularization, graph neural networks, and generative models. Across these documents, the total token count is 1,863 words. This pilot corpus now satisfies the system's minimum ten-document upload threshold while remaining small enough for repeatable evaluation and inspection.
The documents were selected deliberately to cover core AI/ML subtopics that a learner might plausibly study together, while keeping the input format consistent with the system's upload requirements. In other words, the corpus was not sampled randomly from the web: it was constructed as a controlled, topic-diverse test bundle for end-to-end evaluation.

For this pilot run, the keyword extraction stage produced ten final topic phrases after TF-IDF weighting, MMR selection, and post-processing for redundancy reduction, including \emph{neural networks}, \emph{natural language processing}, \emph{machine learning}, \emph{recommendation systems}, \emph{batch normalization}, and \emph{reinforcement learning}. These queries were then used to drive multi-source retrieval over text, video, and code channels. The raw search stage returned 201 unique resources after de-duplication, comprising 89 text items, 77 videos, and 35 code repositories or notebooks. After recommendation and balancing, the final exported package contained 15 resources, split evenly across the three output modalities.
...
Total corpus words & 1,863\\
Final extracted keywords & 10\\
Raw text resources & 89\\
Raw video resources & 77\\
Raw code resources & 35\\
...
The figure therefore highlights two distinct roles of the recommendation stage. The first is \emph{compression}: it reduces 201 candidates to 15 visible outputs. The second is \emph{pedagogical balancing}: it prevents the system from simply returning the most common type of result. In the present implementation, this balancing happens by selecting top resources per modality rather than globally ranking all resources together. This strategy is intentionally simple, but it aligns well with the educational motivation of the project, which is not just to find relevant resources but to provide complementary forms of reinforcement.
...
\hline
Pilot corpus & Supplies ten AI/ML text documents for repeatable end-to-end testing.\\
...
Table~\ref{tab:keyword_quality} summarises the pilot-corpus keyword comparison. Both approaches cover all ten documents, and the TF-IDF + MMR pipeline increases keyword AI relevance from 30\% to 50\%. In this pilot run, both conditions return ten keywords, but the implemented method produces more topic-bearing multi-word phrases.
...
Table~\ref{tab:pipeline_comparison} reports the main resource-quality comparison. The raw search stage returns 201 resources across three modalities. Reducing this set to a balanced top-five-per-type recommendation list yields a 92.54\% reduction in candidate volume, which is substantial from a user-experience perspective because it transforms a large and noisy set into a small, manageable reinforcement package.
...
Two observations are important here. First, even the raw search stage already has high AI relevance (82.59\%), which indicates that the system’s query construction and search-source design are directionally correct. Second, the ranking stage improves both \emph{authority} and measured AI relevance in the final top-$K$ set. Specifically, authority rises from 40.00\% in the unranked top-$K$ set to 66.67\% in the CBF-ranked set, while AI relevance increases from 93.33\% to 100.00\% and valid URLs remain at 100\%.

This behaviour is encouraging for an educational recommender. On the larger ten-document pilot corpus, the ranking stage is no longer merely protecting quality while preserving relevance; it is actively improving both dimensions. The resulting picture is cleaner: ranking primarily improves source quality, while the main compression effect still comes from reducing a large candidate pool to a small balanced reinforcement package. In other words, the recommendation stage is not only filtering for similarity, but also reshaping the final list toward more trustworthy learning resources.

\begin{figure}[!t]
\centering
\includegraphics[width=0.45\textwidth]{figures/evaluation_pipeline_comparison.png}
\caption{Comparison of raw search, unranked top-$K$, and CBF-ranked top-$K$ outputs. Ranking improves both AI relevance and source authority, and the main compression effect comes from reducing the candidate pool from 201 resources to 15.}
...
The pilot evaluation revealed a useful shift between compression and quality improvement. On the ten-document pilot corpus, the unranked top-$K$ resource set already achieved strong AI relevance, but the final ranked set improved the result further. In particular, the ranked set increased authority from 40.00\% to 66.67\% while also increasing measured AI relevance from 93.33\% to 100.00\%. In an educational setting, that combination is desirable: the recommendation stage is not merely filtering items down to a smaller list, but actively reshaping the output toward more trustworthy and more clearly AI-focused resources.
...
The first next step is to scale the current pilot into a larger offline benchmark. In practical terms, this means assembling multiple AI/ML corpora instead of relying on a single ten-document bundle. Each corpus should represent a realistic learner scenario, for example introductory machine learning, neural networks, transformers and NLP, computer vision, or reinforcement learning. For each topic cluster, multiple document sets should be created with different levels of quality and completeness, such as concise lecture notes, fragmented revision notes, textbook extracts, and mixed-source study packs.
...
\begin{itemize}
\item \textbf{Evaluation scope}: The present evaluation is a pilot on a ten-document bundled corpus rather than a full-scale user study, so the reported numbers should be treated as formative evidence.
...
Area & Main finding\\
\hline
Keyword extraction & TF-IDF + MMR preserved 100\% document coverage and improved keyword AI relevance from 30.00\% to 50.00\%.\\
Resource retrieval & Multi-source search returned 201 candidates across text, video, and code resources, demonstrating broad external coverage.\\
Recommendation stage & Final output compressed 201 candidates to 15 balanced recommendations with 92.54\% noise reduction.\\
Ranking effect & CBF-based ranking increased authority from 40.00\% to 66.67\% and AI relevance from 93.33\% to 100.00\%.\\
